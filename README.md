...
- **Source** (a single dropdown) is where the layout **structure** comes from: **Auto (top performer)** — the client's top mined winner; **This client's winning layouts** — a specific mined winner; **Explore — untested** — a family / hand-authored preset (this is how the former "design formats" survive, as typeset presets); or **Inspiration — replicate an ad** — the detected structure of a chosen reference ad. It is populated from the review backend's `/bq` **`list-sources`** action.
...
**Per-region direction (what to generate).** Each **generated** region — one whose asset is an image the pipeline draws (a background, hero, person or product shot, flagged by `image_need` / a generated role) — also shows a **Direction (what to generate)** textarea in its row, so the operator can steer each element of the ad separately rather than only the whole scene (e.g. "the hero is a man in his 40s eating a burger" while the background stays a clean studio). Copy regions get no direction field. Direction is keyed by region id in one **shared map** (the structure, and so the regions, is the same across variants, and the backend applies one direction map across the whole variant matrix), and it rides both the compile and submit requests as **`regionDirection`** ({region id: direction}); a blank direction is dropped rather than sent empty. It persists on the revision as **`region_direction`** (mirrored in the Python `BriefRevision` and `brief_revision.schema.json`, values constrained to strings), so re-opening a saved brief seeds the fields and a re-compile preserves in-progress edits. This is per-region alongside the whole-picture **Creative direction / What you want** box above; both steer the generated picture only, never the copy.

**Generation prompt (what will generate).** A **scene** render also surfaces, per generated image region, the exact **image prompt** that goes to the model, as an editable **Generation prompt** textarea in the compiled variant card. A structured brief never drafts this prompt (the structure owns the rest), so it was hidden before: the backend now computes it at compile (`/compile` returns **`variant.scene_prompts`** as `[{region_id, role, prompt}]`, built by `generate.resolve_scene_prompts`, pure and no spend), so the operator reads and can tweak the prompt before any spend. `readCompiledBrief` sends the edited prompt back as **`variant.scene_prompts`**, and the backend uses it verbatim for that region at generation (still stripped of copy / competitor leakage via `brief.scene_prompt_overrides`) instead of re-deriving it, for the inspiration and winner/explore sources alike. A **typeset** render has no image prompt, so nothing is shown or sent. This is distinct from the per-region **Direction** above: Direction steers how the prompt is composed; the Generation prompt is the final prompt itself.

**Inspiration confirmation (US-022).** For an **Inspiration** source, Compile returns the reference ad's **detected structure** (and a confidence). Before generating, the editor shows a **confirmation gate**: the detected structure as a wireframe, the confidence, and a **Confirm & continue** / **Use preset instead** choice (the preset fallback is offered when the backend flags the detection below its confidence threshold). The submit bar stays hidden until the operator confirms (or picks the preset); confirming reveals the per-region editor. No picker-derived layout is ever applied in inspiration mode — the request carries `source:{kind:"inspiration", ref:<chosen reference uri>}` and never a winner/explore layout. The reference's own copy, colours and brand never transfer: only geometry (roles, boxes, counts) does, and the client's brand and copy are applied at generation. The inspiration picker still marks which statics already carry a structural row (a small **structure** badge) versus one analysed on demand (an **on-demand** badge). A competitor reference is never sent to the image model as pixels — it can only shape an ad through its detected structure.
...
**Source picker + `list-sources` (US-022).** The **Source** dropdown is populated from the review backend's `/bq` **`list-sources`** action, which returns `{ sources: { winners:[…], explore:[…], inspiration:{available} }, renders, explore_prefix }`. Each **winner** carries an `archetype_id`, a `name`, `source_ad_count`, its `structure` and its `default_render`; each **explore** entry carries a `family`, a `preset_id`, its `structure` and `default_render`. The picker offers **Auto (top performer)** (the top winner; `source:{kind:"winner", ref:""}`), the client's **winning layouts** (`source:{kind:"winner", ref:<archetype_id>}`), an **Explore — untested** group of families/presets not already covered by a winner (`source:{kind:"explore", ref:<preset_id or family>}`), and — when `inspiration.available` — an **Inspiration** option. This is how the ten former "design formats" survive: as typeset explore **presets**, each defaulting the render to typeset. The picker degrades to Auto-only if the `list-sources` read fails, so it never blocks the editor.
