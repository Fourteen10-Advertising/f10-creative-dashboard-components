...
`f10-brief-editor.js` adds a **Brief Editor** tab for the F10 internal review app. The tab is laid out as a **numbered top-to-bottom flow** so an operator builds a brief in reading order: **(1) creative axes** (the four canonical dropdowns), **(2) copy** (optional), **(3) creative direction** (optional free text), **(4) inspiration references** (upload / your library / competitors), **(5) compile brief** (the primary action; "save as new revision" sits beside it as an optional secondary action), and **(6) the compiled result**. Loading an existing revision to edit is an optional starting point shown above the flow. At the very top of the panel a **mode toggle** chooses how the operator starts: **From scratch** (the default — build the whole brief: axes, copy, direction, inspiration, exactly as above) or **From inspiration** (the Creative axes and Copy sections are hidden and section 3 is relabelled **What you want**, so the operator just picks an ad and describes it — the backend defaults the axes to the client's winners and auto-writes the copy). The toggle is live (no reload) and nothing is persisted; it defaults to From scratch on load, and is universal (no per-client logic). The chosen mode is carried into the compile/submit contract as a **`mode`** field (`"scratch"` or `"inspiration"`): scratch sends the full inline brief exactly as before **plus** `mode:"scratch"` (purely additive, byte-identical otherwise), while inspiration sends `mode:"inspiration"` with the inline `brief` doc's four axis values and `copy_blocks` **omitted** (the backend fills them), still sending `creativeDirection` and `baseInspirationImageUris`. Steering the axes and copy and saving a **new** revision lets a brief be steered before generation spends without breaking the canonical vocabularies. The editor drives the full **compile, review, edit, submit and generate** flow in the app. **Compile** resolves the on-screen brief with NO spend and returns the exact per-variant image prompts, the copy that will render, the inspiration images with any validation warnings, the target sizes and a cost estimate against the remaining cap. The operator edits the resolved prompts and copy inline, and those edits are exactly what get generated. **Submit** then runs generation server-side on the review backend (the US-001 in-process worker-thread model), enforcing the hard spend cap BEFORE anything runs plus a no-concurrent-run guard; results land back in the panel as thumbnails (see Viewable results below) and the approved brief is saved as a new revision so every run is reproducible. That revision id can alternatively be generated from the CLI (see After save below).
...
**From-inspiration blueprint editor (US-009 to US-012).** In From inspiration mode, Compile seeds a brand-safe **reference blueprint** from the primary picked static: the reference's reconstructed layout structure (a layout family, slot boxes and text-block boxes, all normalised 0..1) plus its declared, closed-vocabulary strategy. It renders as an editable panel above the variants, mirroring the design edit loop, so the operator adjusts the structure and strategy for brand fit and gives a one-click faithful or not-faithful rating before generating. Submit sends the edited blueprint, the seeded baseline and the rating, so generation runs exactly what was approved and the backend logs the from-inspiration success signal (an edit-distance proxy plus the rating). The reference's own copy, colours and brand never enter the blueprint: only geometry and abstract strategy transfer, and the client's brand and copy are applied at generation. The inspiration picker marks which statics already carry a structural row (a small **structure** badge) versus one that will be analysed on demand (an **on-demand** badge); a static with no structure yet still works, analysed on demand, and shows a clear not-yet-available note when analysis is unavailable. The whole path is gated by a per-client rollout flag: with it off, From inspiration stays on the prior holistic behaviour. A competitor reference is never sent to the image model as pixels: it can only shape an ad through its blueprint, and it fails closed if no blueprint is available.
...
**Format picker (image vs typeset design ads).** Above the mode toggle a **Format** picker chooses what kind of ad to make: **Image ad** (the default — a generated scene with the copy laid over it, the full build flow above) or a typeset **design** ad the strategist writes from the client's substance and live scoreboard. The design formats cluster into three shared **chassis** (build the chrome once, each format is a thin content adapter, and they stay a visual family): a **table** (Comparison chart, Feature table), a **card** (Stat card, Testimonial, Offer card) and a **list** (Checklist, FAQ card), plus the native surfaces (**Native UI note**, **Search result** and **Review card**). Picking a design format hides the whole image build flow (mode toggle, revision loader, the brief form and the compiled result) and shows a **Draft ad** button (with an optional creative-direction field), because a design ad needs none of the image flow: the strategist drafts the whole brief (objective, audience, copy, the format's structure, compliance) itself. A design format sends the chosen archetype key as **`archetypeId`** (for example `comparison`, `feature_table`, `stat_card`, `testimonial_card`, `offer_card`, `checklist`, `faq_card`, `native_ui`, `native_ui_search`, `native_ui_review`) on the request; image omits it so the backend auto-picks the mined layout. The **Creative direction** field stays available in design mode as an optional **soft steer** for angle, emphasis and tone — the strategist still uses only the client's real facts and stays within compliance (a design ad has no rendered disclaimer slot, so no efficacy or results claim ever appears in the visible copy). Design ads have a **draft → edit → preview → publish** loop (Phase 2). **Draft ad** compiles the design with **no spend** and returns its full `design_spec`; the drafted ad's wording is shown as **editable fields** (one per copy block: headline, CTA, rows, list items, quotes, answers, values), with a **Preview** and a **Publish** button. Because the *renderer* is deterministic (only the strategist is not), every preview and the publish **re-render the exact edited spec** — the strategist is drafted once and never re-run — so **what publishes equals what was previewed** (no drift). Preview posts the edited spec to the no-spend `/design-preview` route and shows the rendered image. Editing is a new way for a results/efficacy claim to reach an ad that has **no rendered disclaimer slot**, so the backend re-checks compliance on every edited spec at both preview and publish; a failing edit is surfaced **inline** (a 422 with the issues) and nothing renders or spends. **Publish** submits the edited `layoutSpec`, the job renders it verbatim, and the operator reviews and approves the result in the **Review** tab. A **plain** design ad has **no spend** (the render service rasterises typeset HTML, no image model), so drafting and previewing are free. An optional **photo toggle** (`wantImage`) adds a generated, on-brand background image behind the design; that variant costs a small amount to generate under the SpendCap and, unlike a plain design ad, cannot be regenerated for free. The generated imagery is compliance-gated: only lifestyle / product / scene intents, never a generated face and never a before/after transformation. (The image path is unchanged: it still compiles, previews and submits a `compiledBrief` exactly as before.)

**Layout picker (image ads).** For an image ad, a **Layout** dropdown (below the format picker, image-only) chooses which layout the scene is built on, so an operator is no longer limited to the backend's auto-pick of the top layout. It is populated from the review backend's `/bq` `list-archetypes` action and offers three kinds of choice: **Auto (top performer)** — the unchanged default, which omits `archetypeId` so the backend picks the client's top mined layout; **This client's winning layouts** — each of the client's mined archetypes, which pins that layout by sending its `archetype_id`; and **Explore — untested** — every remaining layout family (the ones the client has not won with), sent as `archetypeId:"family:<name>"`, which generates against a neutral skeleton so the team can deliberately test a new layout. The picker is shown only for an image ad **in From-scratch mode** (a design format carries its own fixed layout, and its `archetypeId` still wins). In **From inspiration** mode the layout row is hidden and the choice is reset to Auto, and no layout-derived `archetypeId` is sent — the inspiration ad carries its own structure, so the backend defers to it rather than the picker forcing a layout over it. It degrades to Auto-only if the list read fails, so it never blocks the editor; and a mined family is never double-listed under Explore. Strategy still comes from the client's live winners in every case — only the layout is exploratory.
